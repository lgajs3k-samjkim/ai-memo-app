...
import { MAX_TAGS, mergeTags, suggestMemoTags } from '@/utils/ai'
...
export default function MemoForm({
  ...
  const handleGenerateTags = async () => {
    ...
    try {
      const suggested = await suggestMemoTags(
        formData.title,
        formData.content,
        formData.tags
      )
      setFormData(prev => ({
        ...prev,
        tags: mergeTags(prev.tags, suggested),
      }))
    } catch (error) {
      setTagError(
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label
                ...
                className="block text-sm font-medium text-gray-700 mb-2"
              >
                내용 * <span className="text-gray-400 font-normal">(마크다운)</span>
              </label>
              <MarkdownEditor
                id="content"
            ...
            <div>
              ...
              <div className="flex gap-2 mb-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-orange-500 text-white hover:bg-orange-600 disabled:bg-orange-300 rounded-lg transition-colors whitespace-nowrap"
                  data-testid="ai-generate-tags-btn"
                >
                  {isGeneratingTags ? '생성 중...' : 'AI 태그생성'}
                </button>
              </div>
              {tagError && (
                <p className="text-sm text-red-600 mb-3" data-testid="ai-tags-error">
                  {tagError}
                </p>
              )}
